...
def load_data(data_dir):
    ...
    for filename in os.listdir(data_dir):
        ...
        for x in os.listdir(f):
            ...
            img = cv2.imread(str(os.path.dirname(os.path.abspath(__file__)))+str(os.sep)+str(f)+str(os.sep)+str(x))
...
def get_model():
    ...
    tf.keras.layers.Conv2D(
        36, (3, 3), activation="sigmoid", input_shape=(IMG_WIDTH,IMG_HEIGHT,3)
    ),
